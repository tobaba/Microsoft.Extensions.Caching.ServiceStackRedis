...
namespace Microsoft.Extensions.Caching.ServiceStackRedis
{
    public class ServiceStackRedisCache : IDistributedCache
    {
        ...
        public async Task<byte[]?> GetAsync(string key, CancellationToken token = default)
        {
            ...
            await using var client = await _redisClientsManager.GetClientAsync(token: token);
            if (!await client.ContainsKeyAsync(key, token))
            {
                return null;
            }
        ...
        public async Task SetAsync(string key, byte[] value, DistributedCacheEntryOptions options, CancellationToken token = default(CancellationToken))
        {
            ...
            await using var client = await _redisClientsManager.GetClientAsync(token: token);
            await client.SetEntryInHashAsync(key, nameof(CacheEntry.Value), Encoding.UTF8.GetString(value), token);
        ...
        public async Task RemoveAsync(string key, CancellationToken token = default)
        {
            ...
            await using var client = await _redisClientsManager.GetClientAsync(token: token);
            await client.RemoveAsync(key, token);
        }
